spicy_foods = [
    {
        "name": "Green Curry",
        "cuisine": "Thai",
        "heat_level": 9,
    },
    {
        "name": "Buffalo Wings",
        "cuisine": "American",
        "heat_level": 3,
    },
    {
        "name": "Mapo Tofu",
        "cuisine": "Sichuan",
        "heat_level": 6,
    },
]

def get_names(spicy_foods):
    return [food["name"] for food in spicy_foods]

def get_spiciest_foods(spicy_foods):
    return [food for food in spicy_foods if food["heat_level"] > 5]

def print_spicy_foods(spicy_foods):
    for food in spicy_foods:
        print(f"{food['name']} ({food['cuisine']}) | Heat Level: {'🌶' * food['heat_level']}")

def get_spicy_food_by_cuisine(spicy_foods, cuisine):
    return next((food for food in spicy_foods if food["cuisine"] == cuisine), None)

def print_spiciest_foods(spicy_foods):
    spiciest = get_spiciest_foods(spicy_foods)
    print_spicy_foods(spiciest)

def get_average_heat_level(spicy_foods):
    if not spicy_foods:
        return 0
    total_heat_level = sum(food["heat_level"] for food in spicy_foods)
    return total_heat_level / len(spicy_foods)

def create_spicy_food(spicy_foods, spicy_food):
    return spicy_foods + [spicy_food]

# Dummy data execution
if __name__ == "__main__":
    print("Names of spicy foods:", get_names(spicy_foods))
    print("\nSpiciest foods (heat level > 5):")
    print_spicy_foods(get_spiciest_foods(spicy_foods))
    
    print("\nPrint all spicy foods:")
    print_spicy_foods(spicy_foods)
    
    print("\nGet spicy food by cuisine 'American':")
    print(get_spicy_food_by_cuisine(spicy_foods, "American"))
    
    print("\nPrint only spiciest foods (heat level > 5):")
    print_spiciest_foods(spicy_foods)
    
    print("\nAverage heat level of spicy foods:", get_average_heat_level(spicy_foods))
    
    new_food = {
        "name": "Griot",
        "cuisine": "Haitian",
        "heat_level": 10,
    }
    updated_spicy_foods = create_spicy_food(spicy_foods, new_food)
    print("\nUpdated list of spicy foods after adding 'Griot':")
    print_spicy_foods(updated_spicy_foods)
